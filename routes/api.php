...
// V2 VERSION - VERSION TO APP
Route::group(['prefix' => 'v2','namespace'=>'v2'],function (){
    Route::post('login', 'UserAPIController@login');
    Route::post('register', 'UserAPIController@register');
    // Route::post('send_reset_link_email', 'API\UserAPIController@sendResetLinkEmail');
    Route::post('artisan', 'UserAPIController@artisan'); 

    // LIST OF CATEGORIES
    Route::get('categories','ApiController@getCategories');
    // LIST OF WORKOUTS OF SELEECTED CATEGORY
    Route::get('category_workouts','WorkoutsController@getCategoryWorkouts');
    // Route::get('workouts','ApiController@getWorkouts');
    Route::get('workouts_search','WorkoutsController@getWorkoutsSearch');

    // WORKOUTS ROUTES
    Route::apiResource('workouts', 'WorkoutsController')->except([
        'store','update','destroy'
    ]);

    // AUTHETICATED ROUTES
    Route::group(['middleware' => 'auth:api'],function (){
        Route::post('user', 'UserAPIController@loadUser');
        // LIST ALL PAYMENT METHODS OF USER ON STRIPE
        Route::get('user_payment_methods', 'UserAPIController@userPaymentMethods');
        // ÃDD PAYMENT METHOD TO STRIPE AND ATTACH TO AUTH USER
        Route::post('add_payment_method', 'UserAPIController@addPaymentMethod');
        // ADD SUBSCRIPTION OF PRODUCT TO CUSTOMER AND PAYMENT METHOD
        Route::post('add_subscription', 'UserAPIController@addSubscription');



        // SAVE INFORMATION, TICKET RECEIPT, INFORMATION TO VALIDATE APPLE SUBSCRIPTION PAYMENT
        Route::post('apple_subscription', 'UserAPIController@appleSubscription');
    ...
    Route::get('user_subscriptions','UserAPIController@usersSubscriptions');

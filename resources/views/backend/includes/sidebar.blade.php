    ...
    <nav class="sidebar-nav">
        <ul class="nav">
            ...
            @if((!$logged_in_user->hasRole('student')) && ($logged_in_user->hasRole('teacher') || $logged_in_user->isAdmin() || $logged_in_user->hasAnyPermission(['course_access','lesson_access','test_access','question_access','bundle_access'])))
                ...
                <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/workouts*','user/lessons*','user/tests*','user/questions*','user/live-lessons*','user/live-lesson-slots*']), 'open') }}">
                    ...
                    <ul class="nav-dropdown-items">
                        ...
                        @can('course_access')
                            <li class="nav-item ">
                                <a class="nav-link {{ $request->segment(2) == 'workouts' ? 'active' : '' }}"
                    ...
                    <ul class="nav-dropdown-items">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ active_class(Active::checkUriPattern('admin/settings/zoom-settings*')) }}"
                               href="{{ route('admin.zoom-settings-get') }}">

...
class CoursesController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        // TO ALLOW TEACHER 5 HAVE ACTIONS - VB
        if(auth()->user()->id == '5'){
            $has_view = true;
            $has_edit = true;
            $has_delete = true;
        ...
        return DataTables::of($courses)
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                ...
                $delete = "";
                if ($request->show_deleted == 1) {
                    return view('backend.datatable.action-trashed')->with(['route_label' => 'admin.workouts', 'label' => 'id', 'value' => $q->id]);
                }
                if ($has_view) {
                    $view = view('backend.datatable.action-view')
                        ->with(['route' => route('admin.workouts.show', ['workout' => $q->id])])->render();
                }
                if ($has_edit) {
                    $edit = view('backend.datatable.action-edit')
                        ->with(['route' => route('admin.workouts.edit', ['workout' => $q->id])])
                        ->render();
                    $view .= $edit;
                }
                ...
                if ($has_delete) {
                    $delete = view('backend.datatable.action-delete')
                        ->with(['route' => route('admin.workouts.destroy', ['workout' => $q->id])])
                        ->render();
                    $view .= $delete;
                }
                if ($q->published == 1) {
                ...
                $view .= view('backend.datatable.'.$type)
                    ->with(['route' => route('admin.workouts.publish', ['id' => $q->id])])->render();
                return $view;
            })
            ...
            ->addColumn('status', function ($q) {
                $text = "";
                $text = ($q->published == 1) ? "<p class='text-white mb-1 font-weight-bold text-center bg-dark p-1 mr-1' >" . trans('labels.backend.workouts.fields.published') . "</p>" : "<p class='text-white mb-1 font-weight-bold text-center bg-primary p-1 mr-1' >" . trans('labels.backend.workouts.fields.unpublished') . "</p>";
                if (auth()->user()->isAdmin() || auth()->user()->id == '5') {
                    $text .= ($q->featured == 1) ? "<p class='text-white mb-1 font-weight-bold text-center bg-warning p-1 mr-1' >" . trans('labels.backend.workouts.fields.featured') . "</p>" : "";
                    $text .= ($q->trending == 1) ? "<p class='text-white mb-1 font-weight-bold text-center bg-success p-1 mr-1' >" . trans('labels.backend.workouts.fields.trending') . "</p>" : "";
                    $text .= ($q->popular == 1) ? "<p class='text-white mb-1 font-weight-bold text-center bg-primary p-1 mr-1' >" . trans('labels.backend.workouts.fields.popular') . "</p>" : "";
                }
            ...
            ->editColumn('price', function ($q) {
                if ($q->free == 1) {
                    return trans('labels.backend.workouts.fields.free');
                }
                return $q->price;
            })
...
//                }
        ...
        return redirect()->route('admin.workouts.index')->withFlashSuccess(trans('alerts.backend.general.created'));
    }
    ...
    public function update(UpdateCoursesRequest $request, $id)
    {
        ...
        return redirect()->route('admin.workouts.index')->withFlashSuccess(trans('alerts.backend.general.updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        if ($course->students->count() >= 1) {
            return redirect()->route('admin.workouts.index')->withFlashDanger(trans('alerts.backend.general.delete_warning'));
        } else {
            $course->delete();
        }


        return redirect()->route('admin.workouts.index')->withFlashSuccess(trans('alerts.backend.general.deleted'));
    }
    ...
    public function restore($id)
    {
        ...
        return redirect()->route('admin.workouts.index')->withFlashSuccess(trans('alerts.backend.general.restored'));
    }

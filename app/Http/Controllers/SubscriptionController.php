...
class SubscriptionController extends Controller
{
    ...
    /************ SUBSCRIPTIONS FUNCTIONS FOR APP ************/

    public function showFormApp(Request $request, StripePlan $plan)
    {
        // LOAD User
        $user = User::where('id', '=', $request->input('user_id'))->first();

        if(!isset($user)){
            \Log::info($e->getMessage() . ' --  user not found');
            \Session::flash('failure','Error creating subscription.');
            return redirect()->route('subscriptions.status.fail.app')->withErrors('Error creating subscription.');
        }
        // dd($plan->amount);
        Auth::loginUsingId($request->input('user_id'), $remember = true);

        // CREATE STRIPE SETUP INTENT
        $intent = $user->createSetupIntent();
        
        return view($this->path.'.subscription.form_app', compact('plan', 'intent'));
    }

    /**
     * Process the form
     */
    public function subscribeApp(Request $request, StripePlan $plan)
    {
        $paymentMethod = $request->paymentMethod;
        // grab the user
        $user = $request->user();
        ...
        return redirect()->route('subscriptions.status.success.app');
    }
     ...
     * Update the subscription
     */
    public function updateSubscriptionApp(Request $request, StripePlan $plan)
    {
        // dd("updateSubscriptionApp");
        $user = $request->user();
    ...
    public function statusSuccessApp()
    {

        \Session::flash('success', trans('labels.subscription.done'));
        return view('frontend.subscription.status_success_app');
    }

    public function statusFailApp()
    {
        \Session::flash('failure','Error creating subscription.');
        return view('frontend.subscription.status_fail_app');
    }
}
